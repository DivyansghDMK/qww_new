...
class HolterReplayEngine:
    ...
    def play(self, speed: float = 1.0):
        ...
        if self._playing:
            return
        self._playback_speed = max(0.25, min(speed, 8.0))
        self._playing = True
        self._stop_event.clear()
        self._playback_thread = threading.Thread(
            target=self._playback_loop, daemon=True, name="HolterPlayback"
        )
    ...
    def set_speed(self, speed: float):
        """Change playback speed (0.25x–8x) on the fly."""
        self._playback_speed = max(0.25, min(speed, 8.0))

    def _playback_loop(self):
        """Background thread: advances position and emits frames at real-time rate."""
        # Emit a frame every ~100ms of wall time (10 fps display)
        display_interval = 0.10           # wall seconds per UI update
        while self._playing and not self._stop_event.is_set():
            t_start = time.time()

            # Advance recording position
            self._current_sec += display_interval * self._playback_speed
            ...
            # Fire arrhythmia callback if we're near a known event
            for ev_t, ev_label in self._arrhythmia_events:
                if abs(ev_t - self._current_sec) < display_interval * self._playback_speed:
                    if self._on_arrhythmia_event:
                        try:
                            self._on_arrhythmia_event(ev_t, ev_label)
                        except Exception:
                            pass
