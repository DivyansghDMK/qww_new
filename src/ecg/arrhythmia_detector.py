...
def detect_arrhythmia(beats_list: Sequence[Dict[str, object]], signal_dict: Dict[str, Sequence[float]], fs: float = DEFAULT_FS) -> List[str]:
    ...
    # 1. Initialize Advanced Clinical Engines
    from .arrhythmia_engine import (
        BaselineAnalyzer, TemplateMatcher, TachycardiaAnalyzer, 
        BlockAnalyzer, PatternAnalyzer, PacemakerDetector
    )
    
    baseline_analyzer = BaselineAnalyzer(fs)
    template_matcher = TemplateMatcher(fs)
    tachycardia_analyzer = TachycardiaAnalyzer()
    block_analyzer = BlockAnalyzer(fs)
    pattern_analyzer = PatternAnalyzer()
    pacemaker_detector = PacemakerDetector(fs)

    # 2. Run Modular Analyzers
    # AFib & Flutter (Baseline Frequency Domain)
    fib_flutter = baseline_analyzer.check_atrial_fibrillation_flutter(signal_dict, reference_beats, rr_ms)
    arrhythmias.extend(fib_flutter)
    
    # Heart Blocks (State Machine)
    blocks = block_analyzer.check_av_blocks(reference_beats)
    arrhythmias.extend(blocks)
    
    # Tachycardias (SVT, ATach, VTach)
    tachycardias = tachycardia_analyzer.check_tachycardia(reference_beats)
    arrhythmias.extend(tachycardias)
    
    # Ectopics & PVC Morphologies (Cross-Correlation)
    ectopics = template_matcher.group_and_analyze_ectopics(signal_dict, reference_beats)
    arrhythmias.extend(ectopics)
    
    # Ectopic Sequences (Trigeminy, Bigeminy)
    patterns = pattern_analyzer.check_ectopic_patterns(reference_beats)
    arrhythmias.extend(patterns)
    
    # Pacemaker Spikes (Raw Signal Slew Rate)
    pacemakers = pacemaker_detector.detect_pacemaker(signal_dict, reference_beats)
    arrhythmias.extend(pacemakers)

    # 3. Bundle Branch Logic (Kept internal for simplicity)
    ...
    total_beats = max(len(reference_beats), 1)
    if rbbb_count / total_beats > 0.3: arrhythmias.append("RT bndl branch block") # Match API
    if lbbb_count / total_beats > 0.3: arrhythmias.append("Lt bndl branch block") # Match API
    
    # 3.5. Ventricular Fibrillation Check
    # If the signal is wildly chaotic and fast, the peak detector finds garbage. 
    # The user screenshot showed VFib getting 120bpm with missed beats.
    # We can perform a pure signal-level check for VFib:
    vfib_detected = False
    lead_ii_sig = _safe_array(signal_dict.get("II"))
    if lead_ii_sig.size > fs * 2:
        # Measure chaotic amplitude variation
        zero_crossings = np.sum(np.diff(np.sign(lead_ii_sig - np.mean(lead_ii_sig))) != 0)
        zc_rate = zero_crossings / (lead_ii_sig.size / fs) # Crossings per second
        
        # In true VFib, ZC rate is 5-40 Hz and raw signal amplitude variation is extremely large.
        peak_std = np.std(lead_ii_sig)
        
        if 5.0 < zc_rate < 40.0 and peak_std > 100.0:
            # Further verify no flat baselines exist (VFib is continuous oscillation)
            abs_diff = np.abs(np.diff(lead_ii_sig))
            if abs_diff.size > 0:
                # Normal sinus spends >75% of time with sample diffs < 10 counts. VFib is < 60%.
                flat_ratio = np.sum(abs_diff < 10.0) / abs_diff.size
                if flat_ratio < 0.60:
                    vfib_detected = True

    # 4. Resolve Hierarchical Priorities (Clinical Suppression)
    # E.g., Don't report PAC if we are in AFib
    resolved = []
    
    if vfib_detected:
        resolved.append("ventricular fibrillation 1")
        return resolved # VFib suppresses ALL other findings
        
    for label in arrhythmias:
        if label not in resolved:
            resolved.append(label)
            
    if "Arterial fibrillation 1" in resolved:
        resolved = [r for r in resolved if "PAC" not in r and "PNC" not in r]
